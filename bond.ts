...
export function generateCreateArgs(
    ...
    if (curveData) {
        ...
        if (creatorAllocation > 0) {
            stepRanges.unshift(
                wei(creatorAllocation, tokenType === 'ERC20' ? 18 : 0),
            );
...
export function generateTableData(steps: { x: bigint; y: bigint }[]) {
    ...
    for (let i = 0; i < clonedSteps.length; i++) {
        ...
        obj.start =
            i - 1 < 0 ? 0 : +ethers.utils.formatEther(clonedSteps[i - 1].x);
        obj.end = +ethers.utils.formatEther(clonedSteps[i].x);
        obj.price = +ethers.utils.formatEther(clonedSteps[i].y);
        if (width > 0 && height > 0) {
            const tvl = +ethers.utils.formatUnits(width * clonedSteps[i].y, 36);
            obj.tvl = tvl;
            totalTVL += tvl;
        }
...
const initialMintingPrice = 0.0000001;
const stepCount = 32;
